...
test.describe("location card clock", () => {
  test("shows the city and a live HH:MM local time", async ({ page }) => {
    await page.goto("/");

    const card = page.locator('[data-bento-area="place"]');
    await expect(card).toHaveCount(1);
    await expect(card.getByText("Batangas, PH")).toBeVisible();
    await expect(card.getByText("GMT+8 · Local time")).toBeVisible();
